...
import { Horizon, TransactionBuilder, Networks as SDKNetworks, Asset, Operation, Contract, nativeToScVal, rpc } from '@stellar/stellar-sdk';
...
function App() {
  ...
  const requestDonationConfirmation = (e) => {
    e.preventDefault();
    if (!publicKey) {
  ...
  const donateToCampaign = async () => {
    const amountNum = parseFloat(donationAmount);
    if (!publicKey || isNaN(amountNum) || amountNum <= 0) return;
    ...
    try {
      ...
      let transaction = new TransactionBuilder(sourceAccount, {
        ...
        .addOperation(
          contract.call(
            ...
            nativeToScVal(Math.floor(amountNum * 10000000), { type: 'i128' })
          )
        )
        .setTimeout(30)
        .build();

      transaction = await soroban.prepareTransaction(transaction);
      ...
      setTxStatus('Submitting payment to Soroban Testnet...');

      const txToSubmit = TransactionBuilder.fromXDR(signedXdr, SDKNetworks.TESTNET);
      const response = await soroban.sendTransaction(txToSubmit);
      
      if (response.status === 'ERROR') {
        throw new Error(response.errorResultXdr || 'Soroban transaction failed');
      }
  ...
  const handleSaveToIPFS = async () => {
    ...
    setIpfsUri('');
    try {
      const lastCopy = [...aiMessages].reverse().find(m => m.sender === 'ai' && m.text.includes('Suggested Campaign Copy'));
      const description = lastCopy ? lastCopy.text : 'Stellar Ecosystem Innovation Fund - A decentralized crowdfunding initiative on Soroban.';
      
      const uri = await uploadMetadataToIPFS({
        title: 'Stellar Ecosystem Innovation Fund',
        description,
        tags: ['Soroban', 'Stellar', 'Crowdfunding'],
        timestamp: new Date().toISOString()
      });
      setIpfsUri(uri);
    } catch (err) {
      console.error(err);
      setErrorMessage('Failed to save to IPFS. Check console for details.');
    } finally {
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        <div className="logo-badge">
          ...
          <span className="level-badge yellow-belt">Level 1 Yellow Belt</span>
        </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="wallet-options">
              ...
              <button className="wallet-option-card" onClick={() => connectWithWallet('lobstr', 'Lobstr')}>
                <div className="wallet-option-icon">🦞</div>
                <div className="wallet-option-info">
                  <span className="wallet-option-title">Lobstr</span>
                  <span className="wallet-option-desc">WalletConnect signer</span>
                </div>
                <span className="wallet-option-badge">Mobile</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {showReceiptModal && lastReceipt && (
        <div className="modal-overlay" onClick={() => setShowReceiptModal(false)}>
          <div className="modal-content receipt-modal" onClick={(e) => e.stopPropagation()}>
            <div className="receipt-badge">🎉 Testnet payment confirmed</div>
            <h2>Donation Receipt</h2>
            <div className="receipt-amount">{lastReceipt.amount} XLM</div>
            {lastReceipt.badgeUnlocked && (
              <div className="badge-unlock-banner">🏅 Your local supporter tier is now {lastReceipt.badgeUnlocked}.</div>
            )}
      ...
      {showConfirmModal && (
        <div className="modal-overlay" onClick={() => setShowConfirmModal(false)}>
          <div className="modal-content confirmation-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <h2>Confirm Testnet Payment</h2>
            <p className="modal-subtitle">Review the payment details before your wallet asks you to sign.</p>
            <div className="confirmation-details">
              <div className="receipt-row"><span>Amount</span><strong>{parseFloat(donationAmount).toFixed(1)} XLM</strong></div>
              <div className="receipt-row"><span>Network</span><strong>Stellar Testnet</strong></div>
              <div className="receipt-row"><span>Campaign</span><code>{CAMPAIGN_ADDRESS.slice(0, 8)}...{CAMPAIGN_ADDRESS.slice(-6)}</code></div>
            </div>
            <p className="confirmation-note">This sends Testnet XLM to the campaign address. Review the details in your wallet before signing.</p>
            <div className="receipt-actions">
              <button className="btn btn-primary" onClick={() => { setShowConfirmModal(false); donateToCampaign(); }}>
                Confirm and sign
              </button>
          ...
          <div className="campaign-header-row">
            <div className="campaign-info">
              <div className="tags-row">
                <span className="campaign-tag">Level 1 Stellar Campaign</span>
                {isCreatorVerified && (
            ...
            {!publicKey ? (
              ...
              <div className="dashboard-content">
                  ...
                  <div className="info-card">
                    <div className="balance-header">
                      ...
                      <div className="balance-actions">
                        ...
                        <button className="refresh-btn" onClick={() => { fetchBalance(publicKey); fetchTransactions(publicKey); }} disabled={balanceLoading || transactionsLoading} aria-label="Refresh wallet data" title="Refresh balance and transactions">🔄</button>
                      </div>
                    </div>
                ...
                <div className="tx-section">
                  ...
                  <form onSubmit={requestDonationConfirmation} className="tx-form">
                ...
                <section className="history-section" aria-live="polite">
                  <div className="history-header">
                    <div>
                      <h3>Recent Testnet Transactions</h3>
                      <p>Live history for the connected wallet from Stellar Horizon.</p>
                    </div>
                    <button className="refresh-btn" onClick={() => fetchTransactions(publicKey)} disabled={transactionsLoading} aria-label="Refresh transaction history" title="Refresh transaction history">🔄</button>
                  </div>
                  {transactionsLoading ? (
                    <p className="history-empty">Loading transaction history…</p>
                  ) : recentTransactions.length === 0 ? (
                    <p className="history-empty">No Testnet transactions found for this wallet yet.</p>
                  ) : (
                    <div className="transactions-list">
                      {recentTransactions.map((transaction) => (
                        <a key={transaction.hash} className="transaction-item" href={`https://stellar.expert/explorer/testnet/tx/${transaction.hash}`} target="_blank" rel="noopener noreferrer">
                          <span className={`transaction-status ${transaction.successful ? 'transaction-success' : 'transaction-failed'}`}>{transaction.successful ? '✓' : '!'}</span>
                          <span className="transaction-info">
                            <strong>{transaction.hash.slice(0, 10)}...{transaction.hash.slice(-8)}</strong>
                            <small>{transaction.createdAt.toLocaleString()} · {transaction.memo}</small>
                          </span>
                          <span className="transaction-link">View ↗</span>
                        </a>
                      ))}
                    </div>
                  )}
      ...
      <footer className="app-footer">
        <p>Stellar Level 1 Frontend Challenge • Stellar Testnet & Wallets Kit</p>
      </footer>
    </div>
  );
}
